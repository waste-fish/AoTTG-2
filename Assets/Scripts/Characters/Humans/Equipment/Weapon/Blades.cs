...
namespace Assets.Scripts.Characters.Humans.Equipment.Weapon
{
    public class Blades : Weapon
    {
        ...
        public override void Reload()
        {
            if (!Hero.IsGrounded)
            {
                if (!(Hero.Animation[Hero.ReloadAnimation].normalizedTime < 0.2f || Hero.HasThrownBlades))
                {
            ...
            {
                if (!(Hero.Animation[Hero.ReloadAnimation].normalizedTime < 0.13f || Hero.HasThrownBlades))
                {
                    Hero.HasThrownBlades = true;
                    if (WeaponLeft.activeSelf)
                    {
